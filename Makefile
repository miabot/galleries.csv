...
csv:
	for gallery in $(galleries); do echo $$gallery; curl --silent http://api.artsmia.org/gallery/G$$gallery | jq '.objects' | json2csv -f id,title,artist,date,room | uniq | sort -g > $$gallery.csv; done
